...
const MyTasks = () => {
    ...
    return (
        <div className='m-10'>
            <h1 className='text-center text-5xl font-bold text-blue-950 my-5'>{name}'s Tasks</h1>
            ...
            <table className='w-full bg-white border border-gray-300 text-center overflow-x-scroll'>
                ...
                <tbody>
                    {tasks.filter(task => task?.assignedUser === name).map(task => (
                        <tr
                            key={task?._id}>
                            ...
                            <td className="py-2 px-4 bg-blue-100 border border-gray-500 ">
                                <Link to={`/task/${task?._id}`}><button className='btn btn-outline btn-neutral border-0 text-black'>View Details</button></Link>
                            </td>
